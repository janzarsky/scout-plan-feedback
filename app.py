...
konstruktivní a praktickou zpětnou vazbimport os
import streamlit as st
from pypdf import PdfReader
from google import genai
from google.genai import types
...
4. JAZYK: Odpovídej výhradně v českém jazyce s využitím přirozené české skautské terminologie (oddíl, družina, schůzka, výprava, tábor, skautská výchovná metoda, družinový systém, rádce).

STRUKTURA VÝSTUPU:
Vygeneruj strukturovaný Markdown s následujícími sekcemi:
- 🌟 **Hlavní silné stránky** (Vyzdvihni 2-3 konkrétní positiva plánu)
- 🎯 **Příležitosti k růstu a doporučení** (3-4 prioritní oblasti pro zlepšení)
- ⚠️ **Kontrola bezpečnosti a metodiky** (Vyžadované bezpečnostní plány, chybějící klíčové termíny, rizikové oblasti)
...
def get_gemini_client():
    ...
    if not api_key:
        st.error("⚠️ Chybí proměnná prostředí GEMINI_API_KEY!")
        st.stop()
    return genai.Client(api_key=api_key)

# Header
st.title("⚜️ Automatizovaná zpětná vazba pro skautské plány")
...
if st.button("🚀 Analýza plánu a vygenerování zpětné vazby", use_container_width=True):
    if not plan_bytes and not plan_text.strip():
        st.warning("Pro pokračování prosím nahrajte soubor PDF nebo vložte text plánu.")
    else:
        ...
        with st.spinner("🤖 Analýza plánu podle okresní metodiky..."):
            try:
                ...
                st.error(f"❌ Během zpracování došlo k chybě: {e}")u vedoucím oddílů (často ve věku okolo 20 let).
